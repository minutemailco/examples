...
const ssoLogo = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>`;

// Two SSO buttons: the custom IdP enforces email verification; Google doesn't.
const oauthButtons = (mode = 'Sign in') => `
  <div class="menu">
    <button class="btn-oauth" onclick="location.href='/auth/custom'">${ssoLogo} ${mode} with Acme SSO</button>
    <button class="btn-oauth" onclick="location.href='/auth/google'">${googleLogo} ${mode} with Google</button>
  </div>`;
...
function dashboard(user) {
  ...
  const providerLabel = user.provider === 'custom' ? 'Acme SSO (custom IdP)' : user.provider;
  return layout({
    title: 'Dashboard',
    body: `
      ...
      <dl>
        ...
        <dt>Signed in via</dt><dd>${esc(providerLabel)}</dd>
        <dt>Email verified</dt>
        <dd>${user.emailVerified
          ? '<span class="badge ok">verified</span>'
          : '<span class="badge warn">not verified</span>'}</dd>
        ${claims ? `<dt>Custom claims (from ID token)</dt><dd>${Object.entries(claims).map(([k, v]) => `${esc(k)}: ${esc(v)}`).join(' · ')}</dd>` : ''}
      </dl>
